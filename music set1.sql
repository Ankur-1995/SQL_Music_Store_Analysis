...
select billing_city, sum(total) as c from invoice group by billing_city 
order by c desc;
...
on c.customer_id=i.customer_id group by c.customer_id order by total desc;
...
--Lets invite the artist who have writen the most rock music in our dataset. Write a 
...
--Return all the track names that have a song length longer than the average song lenght
...
--customer name, aritst name and total spent.
